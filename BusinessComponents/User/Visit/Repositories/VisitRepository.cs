...
namespace Tunynet.Common.Repositories
{
    ...
    public class VisitRepository : Repository<Visit>, IVisitRepository
    {
        ...
        public override object Insert(Visit entity)
        {
            ...
            if (visitsIds_IEnu == null)
                visitsIds_IEnu = new List<long>();
            List<long> visitsIds_TopVisit = visitsIds_IEnu.ToList();
            if (visitsIds_TopVisit.Count() == 0 || visitsIds_TopVisit.First() != entity.Id)
        ...
        public override void Update(Visit entity)
        {
            ...
            if (visitsIds_IEnu == null)
                visitsIds_IEnu = new List<long>();
            List<long> visitsIds_TopVisit = visitsIds_IEnu.ToList();
            visitsIds_TopVisit.RemoveAll(n => n == entity.Id);
            visitsIds_TopVisit.Insert(0, entity.Id);
            cacheService.Set(cacheKey_TopVisit, visitsIds_TopVisit, CachingExpirationType.UsualObjectCollection);
        ...
        public IEnumerable<long> GetVisitorAlsoVisitedObjectIds(string tenantTypeId, long toObjectId, int topNumber)
        {
            List<long> list = null;
            string cacheKey = string.Format("VisitorAlsoVisitedObjectIds::tenantTypeId:{0}-toObjectId:{1}", tenantTypeId, toObjectId);
            list = cacheService.Get<List<long>>(cacheKey);
            if (list == null)
            {
                var sql_select = PetaPoco.Sql.Builder;
                sql_select.Select("ToObjectId")
                          .From("tn_Visit")
                          .Where("VisitorId in (select VisitorId from tn_Visit where TenantTypeId=@0 and ToObjectId=@1)", tenantTypeId, toObjectId)
                          .Where("TenantTypeId = @0", tenantTypeId)
                          .Where("ToObjectId != @0", toObjectId)
                          .OrderBy("ToObjectId desc");

                IEnumerable<long> toObjectIds = CreateDAO().FetchTop<long>(SecondaryMaxRecords, sql_select);
                if (toObjectIds != null)
                    list = toObjectIds.Distinct().ToList();

                cacheService.Add(cacheKey, list, CachingExpirationType.UsualObjectCollection);
            }
            return list.Take(topNumber);
        }
        ...
        private IEnumerable<long> GetTopVisitIds(string tenantTypeId, long toObjectId)
        {
            ...
            if (peic == null)
            {
                ...
                IEnumerable<long> visitIds = CreateDAO().FetchTop<long>(SecondaryMaxRecords, sql_select);
                if (visitIds != null)
        ...
        public override IEnumerable<Visit> PopulateEntitiesByEntityIds<T>(IEnumerable<T> entityIds)
        {
            var visits = base.PopulateEntitiesByEntityIds<T>(entityIds);
            var userService = DIContainer.Resolve<IUserService>();
            userService.GetUsers(visits.Select(n => n.VisitorId));
            userService.GetUsers(visits.Where(n => n.TenantTypeId == TenantTypeIds.Instance().User()).Select(n => n.ToObjectId));
            return visits;
        }


        public void DeleteCachekey(string tenantTypeId, long toObjectId)
        {
            string cacheKey = GetCacheKey_TopVisits(tenantTypeId, toObjectId);
            cacheService.Add(cacheKey, null, CachingExpirationType.UsualObjectCollection);
        }
    }
}
