...
namespace Spacebuilder.Common
{
    ...
    public class MessageCenterController : Controller
    {
        ...
        public ISettingsManager<MessageSettings> messageSettingsManager { get; set; }
        ...
        public TenantTypeService tenantTypeService { get; set; }
        ...
        public ActionResult _ListMessages(string spaceKey, long sessionId, long userId, bool isShowAll = false)
        {
            ...
            List<Message> iMessages = new List<Message>();
            if (messageSession != null)
            {
                if (isShowAll)
                {
                    List<Message> messages = messageService.GetTops(sessionId, messageSession.MessageCount).ToList();
                    iMessages = messages.Skip<Message>(50).ToList();
                }
                else
                {
                    iMessages = messageService.GetTops(sessionId, 50).ToList();
                }
        ...
        public ActionResult ListMessages(string spaceKey, long sessionId)
        {
            ...
            ViewData["displayName"] = otherUserName;
            ViewData["session"] = messageSession;
        ...
        public ActionResult ListCommentsInBox(string spaceKey, int? pageIndex, string tenantTypeId = "")
        {
            ...
            IEnumerable<ApplicationBase> ownerApps = applicationService.GetInstalledApplicationsOfOwner(PresentAreaKeysOfBuiltIn.UserSpace, userId);
