...
{
    //todo:zhengw:根据ObejectId获取评论列表，以及根据OjectId删除评论时，并没有限制租户类型Id
    /// <summary>
    /// CommentRepository
    /// </summary>
    public class CommentRepository : Repository<Comment>, ICommentRepository
    {
        ...
        public void DeleteTrashDatas()
        {
            ...
            foreach (var tenantType in tenantTypes)
            {
                ...
                sqls.Add(Sql.Builder.Append("delete from tn_Comments")
                                    .Where("not exists (select 1 from " + pd.TableName + " where CommentedObjectId = " + pd.PrimaryKey + ") and TenantTypeId = @0"
        ...
        /// <returns></returns>
        public PagingDataSet<Comment> GetUserComments(long? ownerId, long? userId, string tenantTypeId, DateTime? startDate, DateTime? endDate, int pageIndex, bool? isMobileClient = null)
        {
            ...
            if (isMobileClient != null && isMobileClient.Value)
                sql.Where("TenantTypeId in (@0,@1,@2)", "100101", "100201", "100302");
            ...
            else
            {
                pds = GetPagingEntities(PageSize, pageIndex, CachingExpirationType.ObjectCollection,
                 () =>
                 {
                     ...
                     cacheKey.AppendFormat("IsMobileClient-{0}", isMobileClient);
        ...
        /// <param name="tenantTypeId"></param>
        /// <returns></returns>
        public long GetUserCommentCount(long userId, string tenantTypeId)
        {
            long commentCount = 0;
            string cacheKey = string.Format("GetUserCommentCount::UserId-{0}:tenantTypeId-{1}", userId, tenantTypeId);
            object commentCountObj = cacheService.Get(cacheKey);
            if (commentCountObj == null)
            {
                ...
                commentCount = CreateDAO().FirstOrDefault<long>(sql);
                cacheService.Add(cacheKey, commentCount, CachingExpirationType.UsualSingleObject);
            }
            else
                long.TryParse(commentCountObj.ToString(), out commentCount);
            return commentCount;
        }
    }
}
