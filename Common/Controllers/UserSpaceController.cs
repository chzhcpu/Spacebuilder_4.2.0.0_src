...
namespace Spacebuilder.Common
{
    ...
    public class UserSpaceController : Controller
    {
        ...
        public ISettingsManager<LogoSettings> logoSettingsManager { get; set; }
        ...
        public InvitationService invitationService { get; set; }
        ...
        /// <param name="showOperation">显示操作（如果显示操作就不会显示基础导航）</param>
        [HttpGet]
        ...
        public ActionResult _MyHomeActivities(string spaceKey)
        {
            ...
            IEnumerable<ApplicationBase> applications = applicationService.GetAll(true);
        ...
        /// </summary>
        [HttpPost]
        public ActionResult DeleteHomeVisitors(string spaceKey, int pageIndex = 1, int topNumber = 9)
        {

            long id = Request.Form.Get<long>("id", 0);
            visitService.Delete(id);
            long userId = UserIdToUserNameDictionary.GetUserId(spaceKey);
            visitService.DeleteCachekey(userId);
            ...
            return View("_HomeLastVisitList", visitsArry[pageIndex - 1]);
        }
        ...
        {
            long userId = UserIdToUserNameDictionary.GetUserId(spaceKey);
            //获取最新动态，并排除掉用户发布的微博动态
            IEnumerable<Activity> newActivities = activityService.GetNewerActivities(userId, lastActivityId, applicationId).Where(n => n.UserId != userId || n.TenantTypeId != "100101");
        ...
        /// 首页动态列表
        /// </summary>
        public ActionResult _MyTimeline(string spaceKey, int? pageIndex, int? applicationId, long? groupId, bool? isOriginalThread, MediaType? mediaType, string microblogType = null)
        {
        ...
        /// <returns></returns>
        private SelectList MicroblogType(string selectedValue)
        {
        ...
        /// <returns></returns>
        public ActionResult _UserActivities(string spaceKey, int? pageIndex, int? applicationId = null)
        {
            User user = userService.GetFullUser(spaceKey);
            if (user == null)
        ...
        public JsonResult _EditAvatar(string spaceKey)
        {
            ...
            if (Request.Files.Count > 0 && Request.Files["Filedata"] != null)
            {
                ...
                {
                    userService.UploadOriginalAvatar(userId, Request.Files["Filedata"].InputStream);
