...
namespace Spacebuilder.Common
{
    ...
    public class UserService : IUserService
    {
        ...
        public void RewardAndPunishment(long userId, string description, int experiencePoints, int reputationPoints, int tradePoints, bool isIncome)
        {
            ...
            if (user == null)
                return;
            EventBus<IUser, RewardAndPunishmentUserEventArgs>.Instance().OnBefore(user, new RewardAndPunishmentUserEventArgs(experiencePoints, reputationPoints, tradePoints, 0, 0, 0,EventOperationType.Instance().RewardAndPunishUsers()));
            userRepository.RewardAndPunishment(user, description, experiencePoints, reputationPoints, tradePoints, isIncome);
            EventBus<IUser, RewardAndPunishmentUserEventArgs>.Instance().OnAfter(user, new RewardAndPunishmentUserEventArgs(experiencePoints, reputationPoints, tradePoints, 0, 0, 0,EventOperationType.Instance().RewardAndPunishUsers()));
