...
namespace Spacebuilder.Common
{
    ...
    public class FollowController : Controller
    {
        ...
        public IEnumerable<IEmailContactAccessor> contactAccessors { get; set; }
        ...
        public InviteFriendService inviteFriendService { get; set; }
        ...
        public UserSettings userSettings = DIContainer.Resolve<ISettingsManager<UserSettings>>().Get();
        ...
        public JsonResult BatchFollow(string spaceKey)
        {
            ...
            categoryService.AddItemsToCategory(followedUserIds, groupId);
            ...
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                if (categoryIds != null && categoryIds.Count() > 0)
                {
                    foreach (var followedId in userIds)
                    {
                        ...
                        categoryService.AddCategoriesToItem(categoryIds, follow.Id);
                    }
                }
        ...
        public JsonResult AddFollowedUser(string spaceKey, long followedUserId)
        {
            ...
            if (!followService.IsFollowed(currentUserId, followedUserId))
            {
                result = followService.Follow(UserContext.CurrentUser.UserId, followedUserId, false);
                if (!result)
                {
            ...
            if (categoryIds != null && categoryIds.Count() > 0)
            {
                categoryService.AddCategoriesToItem(categoryIds, followId);//为被关注用户设置分组
            }
        ...
        public JsonResult CreateNewGroupInList(string spaceKey, string groupName, long followedUserId)
        {
            ...
            categoryService.AddCategoriesToItem(categoryIds, follow.Id);//为被关注用户设置分组
        ...
        public JsonResult SetGroupForFollowedUser(string spaceKey, int userGroupId, long followedUserId, long followId, bool isChecked)
        {
            ...
            if (isChecked)
                ...
                categoryService.DeleteItemInCategory(userGroupId, followId);
        ...
        public JsonResult QuietlyFollow(string spaceKey, long? followedUserId)
        {
            ...
            else if (DIContainer.Resolve<Authorizer>().Follow(followedUserId.Value))
            {
        ...
        /// <returns></returns>
        public ActionResult _SpaceHomeMore(string spaceKey, int type = 0, int pageIndex = 1)
        {
            ...
            switch ((SpaceHomeMoreType)type)
            {
                case SpaceHomeMoreType.FollowedUsersFromUser:
                    ...
                    ids = followService.GetTopFollowedUserIdsFromUser(currentUser.UserId, user.UserId, pageSize, pageIndex);
                    break;

                case SpaceHomeMoreType.FollowedUsersOfFollowers:
                    ids = followService.GetTopFollowedUserIdsOfFollowers(user.UserId, currentUser.UserId, pageSize, pageIndex);
                    break;

                case SpaceHomeMoreType.TogetherFollowedUsers:
                    ids = followService.GetTogetherFollowedUserIds(currentUser.UserId, user.UserId, pageSize, pageIndex);
                    break;
            }
