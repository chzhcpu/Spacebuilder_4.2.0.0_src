...
namespace Tunynet.Common
{
    ...
    public class RatingRepository : Repository<Rating>, IRatingRepository
    {
        ...
        private ISettingsManager<RatingSettings> ratingSettingsManager = DIContainer.Resolve<ISettingsManager<RatingSettings>>();
        ...
        /// <returns></returns>
        public int GetRatingCount(long objectId, string tenantTypeId)
        {
            StringBuilder cacheKey = new StringBuilder("GetRatingCount::");
            cacheKey.AppendFormat("ObjectId-{0}:", objectId);
            cacheKey.AppendFormat("TenantTypeId-{0}", tenantTypeId);
            Object count = cacheService.Get<Object>(cacheKey.ToString());
            if (count == null)
            {
                Sql sql = Sql.Builder;
                sql.Append("select RateCount from tn_Ratings");
                sql.Where("TenantTypeId = @0", tenantTypeId)
                    .Where("ObjectId = @0", objectId);
                count = CreateDAO().ExecuteScalar<int>(sql);
                cacheService.Set(cacheKey.ToString(), count, CachingExpirationType.UsualSingleObject);
            }
            return (int)count;
        }
        ...
        /// <param name="tenantTypeId">租户Id</param>
        /// <returns></returns>
        public int GetRatingCountForVideo(long ownerId, string tenantTypeId)
        {
            StringBuilder cacheKey = new StringBuilder("GetRatingCountForVideo::");
            cacheKey.AppendFormat("ownerId-{0}:", ownerId);
            cacheKey.AppendFormat("TenantTypeId-{0}", tenantTypeId);
            Object count = cacheService.Get<Object>(cacheKey.ToString());
            if (count == null)
            {
                Sql sql = Sql.Builder;
                sql.Append("select count(1) from tn_Ratings");
                sql.Where("TenantTypeId = @0", tenantTypeId)
        ...
        /// <param name="tenantTypeId">租户Id</param>
        /// <returns></returns>
        public float GetRatingResult(long objectId, string tenantTypeId)
        {
            StringBuilder cacheKey = new StringBuilder("GetRatingResult::");
            cacheKey.AppendFormat("ObjectId-{0}:", objectId);
            cacheKey.AppendFormat("TenantTypeId-{0}", tenantTypeId);
            Object result = cacheService.Get<Object>(cacheKey.ToString());
            if (result == null)
            {
                int count = GetRatingCount(objectId, tenantTypeId);
                if (count > 0)
                {
                    Sql sql = Sql.Builder;
                    sql.Append("select  AVG(Comprehensive) from tn_Ratings");
                    sql.Where("TenantTypeId = @0", tenantTypeId)
                        .Where("ObjectId = @0", objectId);
                    result = CreateDAO().FirstOrDefault<float>(sql);
                    cacheService.Set(cacheKey.ToString(), result, CachingExpirationType.UsualSingleObject);
                }
                else
                {
                    return 0;
        ...
        /// <param name="tenantTypeId">租户Id</param>
        /// <returns></returns>
        public float GetRatingResultForVideo(long ownerId, string tenantTypeId)
        {
            StringBuilder cacheKey = new StringBuilder("GetRatingResultForVideo::");
            cacheKey.AppendFormat("ownerId-{0}:", ownerId);
            cacheKey.AppendFormat("TenantTypeId-{0}", tenantTypeId);
            Object result = cacheService.Get<Object>(cacheKey.ToString());
            if (result == null)
            {
                int count = GetRatingCountForVideo(ownerId, tenantTypeId);
                if (count > 0)
                {
                    Sql sql = Sql.Builder;
                    sql.Append("select  AVG(RateSum) from tn_Ratings");
                    sql.Where("TenantTypeId = @0", tenantTypeId)
                        .Where("OwnerId = @0", ownerId);
                    result = CreateDAO().FirstOrDefault<float>(sql);
                    cacheService.Set(cacheKey.ToString(), result, CachingExpirationType.UsualSingleObject);
