...
namespace Spacebuilder.Common
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login()
        {
            ...
            {
                if (!returnUrl.ToLower().StartsWith(Tunynet.Utilities.WebUtility.HostPath(Request.Url)))
                    return Redirect(SiteUrls.Instance().SystemMessage(TempData, new SystemMessageViewModel
                    {
                        StatusMessageType = StatusMessageType.Hint,
                        Title = "将要第三方网站，继续访问可能存在风险",
                        Body = "",
                        BodyLink = new Dictionary<string, string> { { "继续访问", returnUrl } }
                    }));
            }
        ...
        /// <returns></returns>
        [HttpGet]
        public JsonResult ValidateNickNameForEditUserProfile(string nickName, long userId)
        {
            IUser currentUser = UserContext.CurrentUser;
            if (currentUser == null)
                return Json("您还没有登录，请登录之后重试", JsonRequestBehavior.AllowGet);
            string errorMessage;
            bool valid = Utility.ValidateNickName(nickName, out errorMessage, userId);
            if (valid)
                return Json(true, JsonRequestBehavior.AllowGet);
            return Json(Tunynet.Utilities.WebUtility.HtmlDecode(errorMessage), JsonRequestBehavior.AllowGet);
        }
