...
namespace Tunynet.Common.Repositories
{
    ...
    public class AdvertisingPositionRepository : Repository<AdvertisingPosition>, IAdvertisingPositionRepository
    {
        ...
        {
            string cacheKey = RealTimeCacheHelper.GetListCacheKeyPrefix(CacheVersionType.AreaVersion, "PositionId", positionId) + "AdvertisingsByPositionId::isEnable-" + isEnable;
            var advertisings = cacheService.Get<List<Advertising>>(cacheKey);
            if (advertisings == null)
            {
                Sql sql = Sql.Builder
                            ...
                            .Where("tn_Advertisings.EndDate>@0", DateTime.Now);
                if (isEnable.HasValue)
                {
                    sql.Where("tn_Advertisings.IsEnable=@0", isEnable.Value);
                }
                advertisings = CreateDAO().Fetch<Advertising>(sql);
                cacheService.Add(cacheKey, advertisings, CachingExpirationType.RelativelyStable);
            }
            return advertisings;
        }
    }
