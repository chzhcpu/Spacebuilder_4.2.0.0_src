    ...
    public class SensitiveWordRepository : Repository<SensitiveWord>, ISensitiveWordRepository
    {
        ...
        public IEnumerable<SensitiveWord> GetSensitiveWords(string keyword, int? typeId)
        {
            ...
            if (string.IsNullOrEmpty(keyword))
            {
                ...
                words = cacheService.Get<List<SensitiveWord>>(cacheKey.ToString());
                if (words == null)
                {
                    words = dao.Fetch<SensitiveWord>(sql);
                    cacheService.Add(cacheKey.ToString(), words, CachingExpirationType.ObjectCollection);
                }
            }
            else
            {
                sql.Where("Word like @0", "%" + StringUtility.StripSQLInjection(keyword) + "%");
                words = dao.Fetch<SensitiveWord>(sql);
            }
