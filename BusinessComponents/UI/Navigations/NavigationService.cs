...
namespace Tunynet.UI
{
    ...
    public class NavigationService
    {
        ...
        private ApplicationService ApplicationService = DIContainer.Resolve<ApplicationService>();
        ...
        protected virtual IEnumerable<Navigation> GetNavigations(string presentAreaKey, long ownerId)
        {
            ...
            List<Navigation> navigations = cacheService.Get<List<Navigation>>(cacheKey);
            if (navigations == null)
            {
                ...
                navigations = navs.OrderBy(n=>n.DisplayOrder).ToList();
        ...
        {
            return EntityData.ForType(typeof(Navigation)).RealTimeCacheHelper.GetListCacheKeyPrefix(CacheVersionType.AreaVersion, "OwnerId", ownerId)
            + string.Format("Navigations::PresentAreaKey-{0}", presentAreaKey);
        }
