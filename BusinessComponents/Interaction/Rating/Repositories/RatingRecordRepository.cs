...
namespace Tunynet.Common
{
    ...
    public class RatingRecordRepository : Repository<RatingRecord>, IRatingRecordRepository
    {
        ...
        /// <returns></returns>
        public RatingRecord GetUserRatingRecords(long objectId, string tenantTypeId, long userId)
        {
            StringBuilder cacheKey = new StringBuilder("GetUserRatingRecords::");
            cacheKey.AppendFormat("objectId-{0}:userId-{1}", objectId, userId);
            RatingRecord ratingRecord = cacheService.Get<RatingRecord>(cacheKey.ToString());
            if (ratingRecord == null)
            {
                Sql sql = Sql.Builder;
                sql.Select("tn_RatingRecords.*")
                    .From("tn_RatingRecords")
                    .Where("ObjectId=@0", objectId)
                    .Where("TenantTypeId=@0", tenantTypeId)
                    .Where("UserId=@0", userId);
                ratingRecord = CreateDAO().FirstOrDefault<RatingRecord>(sql);
                if (ratingRecord != null)
                {
                    cacheService.Set(cacheKey.ToString(), ratingRecord, CachingExpirationType.UsualSingleObject);
                }
            }
            return ratingRecord;
        }
