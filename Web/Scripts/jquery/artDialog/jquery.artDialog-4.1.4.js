...
//解决Bundle和artDialog的路径冲突问题
(function (config) {
    if (config.path.indexOf("/Bundle") > 0)
        config.path = config.path.substring(0, config.path.indexOf("/Bundle")) + "/Scripts/jquery/artDialog";
    config.padding = null;
    window.alert = function (content, $ok) {
        return artDialog({
            id: 'Alert',
            icon: 'warning',
            fixed: true,
            lock: true,
            content: '<div class="tn-dialog-tips">' + content + '</div>',
            ok: $ok || true
        });
    };
})(art.dialog.defaults);